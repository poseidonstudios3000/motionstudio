import type { BrandKind, SceneSpec, TimedWord, VisualKind } from "./motion-composition";

export type DetectedLanguage = "EN" | "DE";
...
const classify = (text: string, index: number, brands: BrandKind[], platforms: SceneSpec["platforms"]): VisualKind => {
...
export const planStoryboard = ({
  ...
  const scenes = beats.map((beat, index) => {
    ...
    const kind = classify(beat.text, index, brands, platforms);
    ...
    return {
      ...
      title: titleFromText(beat.text, language === "DE" ? "Deine Kernidee" : "Your key idea"),
      detail: beat.text,
      brand: brands[0],
      brands,
      platforms,
      metric,
      ...route,
      ctaLabel: language === "DE" ? "Schreib einen Kommentar" : "Leave a comment",
    } satisfies SceneSpec;
  });
